...
def OASIS_example(dataset, update_metric, update_lsh):
    ...
    for i in range(X_test.shape[0]):
        ...
        lsh_index = np.argsort(helpers.euclidean_distances(new_element, training_set_[candidate]))[0, :100]
        result_acc += np.sum(y[candidate][lsh_index] == y_test[i])
    ...
    print ("The accuracy of KNN query when n = 100 is:", result_acc/len(y_test))
...
if __name__ == "__main__":
    ...
    if dataset == "made":
        X, y, X_test, y_test = create_dataset_made_.create_dataset(n_instances=10000, n_testing=1000)
    elif dataset == "hepmass":
        X, y, X_test, y_test = create_dataset_hepmass.create_dataset(n_instances = 11000, n_testing= 1000, random_state=200)

    print ("The number of testing queries is:", X_test.shape[0])

    OASIS_example(dataset, update_metric="none", update_lsh="online")
    print (" ")
